#!/usr/bin/env bash
#
# repl_sampler_standby.sh  —  jalankan DI STANDBY SURABAYA (background/systemd)
# -----------------------------------------------------------------------------
# Sampling berkala metrik sisi apply -> tulis RAW CSV.
# Saat time_lag > ambang, lakukan BURST CAPTURE (snapshot detail momen insiden).
#
...
IN_REC="$($PSQL -c 'SELECT pg_is_in_recovery();' 2>/dev/null)" || { echo "ERROR: gagal konek psql"; exit 1; }
[ "$IN_REC" = "t" ] || { echo "ERROR: ini bukan STANDBY (in_recovery=$IN_REC)."; exit 1; }
...
trap 'echo "[stop] sampler standby dihentikan."; exit 0' INT TERM

# %util tertinggi + device + await dari satu sampel iostat 1 detik
iostat_peak() {
...
# rtt(ms) retrans(total) dari socket replikasi (walreceiver -> primary:5432)
sock_info() {
    local out rtt retr
...
echo "[start] sampler STANDBY -> $CSV (interval ${INTERVAL}s, ambang lag ${THRESHOLD_LAG_S}s)"
...
while :; do
    ...
    [ -z "${recv:-}" ] && { echo "[warn] query gagal"; sleep "$INTERVAL"; continue; }
    ...
    # CPU proses startup (redo) — delta /proc, akurat per-interval
    cpu="-"
    ...
    if python3 -c "exit(0 if float('${tlag:-0}')>${THRESHOLD_LAG_S} else 1)" 2>/dev/null && [ $((EPOCH - last_burst)) -ge "$BURST_COOLDOWN" ]; then
        ...
        BF="${OUTDIR}/burst_standby_$(date '+%Y%m%d_%H%M%S').txt"
        {
            ...
            echo "--- cpu per-core ---"; command -v mpstat >/dev/null && mpstat -P ALL 1 3 2>&1
            echo "--- socket replikasi ---"; ss -tinp '( dport = :5432 or sport = :5432 )' 2>&1 | sed -E 's/(password=)[^ ]*/\1***/Ig'
        } > "$BF"
